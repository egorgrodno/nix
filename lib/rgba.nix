# Translucent form of a `theme` color, for the GTK stylesheets. GTK CSS cannot
# add an alpha channel to a hex literal, and a hand-written `rgba(...)` keeps
# the old color silently when the theme moves.
lib: hex: alpha:
let
